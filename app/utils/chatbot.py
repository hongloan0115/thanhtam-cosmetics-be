...
# from app.utils.chatbot import get_basic_products_info
...
#                 products = get_products_with_details(db, **args)
...
#                 existence_info = check_products_exist(db, keywords=keywords)
                
#                 if existence_info["has_products"]:
#                     products = get_basic_products_info(db, keywords=keywords, limit=5)
...
#             context_data = get_products_for_context(db, keywords)
            
#             if context_data:
#                 advice_response = call_gpt(message, context_data)
                
#                 # Lấy một vài sản phẩm để hiển thị kèm theo lời tư vấn
#                 products = get_basic_products_info(db, keywords=keywords, limit=3)
#                 result = [
#                     thinking_response,
